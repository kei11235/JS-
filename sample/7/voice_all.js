...
const make = function (s, ws, ss) {
	// オシレーター・パッチを作る
	const osc = s.makeOsc({ type: "saw" });
	const noise = s.makeNoise();
	...
	// パッチをつなげる
	s.connect([osc, noise], [bpf1, bpf2], wave, spec, s.speaker());
	// まとめて返す
	return { osc, noise, bpf1, bpf2 };
};
...
const tune = function (inst, t, g, f, sg, sf1, sf2) {
	inst.osc.gain(g, t);
	inst.osc.freq(f + CALC.rand(30), t);
	inst.noise.gain(g * sg / 100, t);
