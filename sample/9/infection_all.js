...
const drawHuman = function (p) {
	...
	r.edge(PATH.triangleEdge(4, d.sick / 1000));
	if (d.recovered) {
		r.fill().color("Blue");
	} else {
		r.fill().color("White");
...
const toMoveToVacantArea = function (h, dm) {
	const deg = getCrowdedDirection(dm, h.x(), h.y());
	const m = h.motion();
	m.direction(deg + 180);
	m.go(10);
	m.doLater(toMoveToVacantArea, [h, dm]);
};
